...
> **Last updated: 2026-09-24.** `1.40.0` = **workouts and the AI trainer REMOVED (owner decision): fitness moves to its own app, the way recipes and pantry did, and Conjure Health is calorie tracking and food entry.** What stays is exercise on the calorie ring: Apple Health / wearable burns, exercise added by hand, and the `logWorkout` action (which now keeps `type` + `durationMin`) — the way the new fitness app feeds the ring. The ring's Exercise row opens the Exercise screen to review, correct and add; Settings → Reset shows it as "Exercise". Also kept: the plan's weekly movement goal and "Ask about food". Removed: the workout library + guided player, cardio/GPS tracking (and the `native.location` permission), the adaptive program + benchmarks + groups, the trainer coach (chat, check-ins, reflections, memory), exercise explainers, and the `COACH_AND_WORKOUTS_ENABLED` flag; plans are food-only (no workout goals, no injuries / experience / training-days questions). Stored data is untouched: old plans keep `program`, old sessions keep their sets, and `coach.json` stays until the user clears their plan or everything. **The last commit with all of the workout code is `caeac19`** (1.38.2, `conjureos-fitness` `main` when the app moved; it is in this repo's history too) — copy from there for the fitness app. Also the move: Conjure Health now lives in `conjureos-health` (full history imported), and `conjureos-fitness` is left for the fitness app; the slug stays `fitness`. Bundle 1,036 → 869 kB.
>
> Prior (2026-08-04): `1.21.0` = **coach + workout program PAUSED (owner decision); Conjure Health ships as a focused weight-loss + nutrition tracker.** One flag, `COACH_AND_WORKOUTS_ENABLED` in `src/features/flags.ts`, hides the Workouts tab + library, the Coach tab + Plan-tab launcher, the evening check-in, the Plan tab's program/benchmark section, and the wizard's mode picker (plans are forced `eat_better`). NOTHING is deleted: all code + 86 tests stay green, existing plans keep `program`, `coach.json` and session history are untouched, and no data migrates — flipping the flag back to `true` restores everything. Deliberately STILL ON: Apple Health / wearable exercise calories (they move the calorie budget, so they're a nutrition feature) and the `logWorkout` action; the ring's Exercise row now opens an exercise-only view (`WorkoutsScreen exerciseOnly`) to edit/remove those burns. Also fixed a pre-existing gap this exposed: food-only plans had NO 'Edit plan' entry point (it lived inside ProgramSection, which renders nothing without a program) — new `PlanHeaderSection` always provides it. Bundle 710→662 kB. Revival checklist lives in the flags.ts doc comment.
>
...
- **Frontend:** this repo (`conjureos-health`; it was `conjureos-fitness` until
  2026-09-24), Vite + React + TypeScript. The version is in `package.json`.
